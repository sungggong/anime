...
3. `index.html`의 `<head>`에 있는 Google tag ID도 같은 값으로 유지합니다. 현재 사이트는 `G-ZQ8WZ0SKF4`가 적용되어 있습니다.
...
측정 ID가 설정되어 있어도 Google Consent Mode 기본값은 `denied`입니다. 방문자가 동의 배너에서 허용을 누른 뒤에만 `analytics_storage`를 `granted`로 업데이트하고 커스텀 이벤트/페이지뷰를 전송합니다. 거절하면 대기 중인 이벤트도 삭제됩니다.
